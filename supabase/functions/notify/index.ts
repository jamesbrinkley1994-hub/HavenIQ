...
async function sendEmail(to: string, subject: string, html: string, from = 'HavenIQ <notifications@haveniq.app>') {
...
serve(async (req) => {
  try {
    ...
    // ── Feedback → HavenIQ team ───────────────────────────
      ...
      // Uses Resend's built-in testing address (always verified, no domain
      // setup needed) since notifications@haveniq.app isn't verified yet.
      // Scoped to feedback only — booking/reply emails are untouched.
      const res = await sendEmail('Haven.IQ@outlook.com',
        ...
        'HavenIQ Feedback <onboarding@resend.dev>'
      )
      if (!res.ok) {
        const errText = await res.text()
        return new Response(JSON.stringify({ error: `Feedback email failed: ${errText}` }), { status: 500 })
      }
    }
