...
const ALL_NAV = [
  ...
  { id:'feedback',   label:'Feedback',   path:'/haven/app/feedback',   icon:FeedbackIcon, section:'Account' },
]
...
export default function AppShell({ title, topbarRight, children }) {
  ...
  const { business, user, signOut } = useStore()
      ...
      <aside className="sidebar">
        ...
        <div className="sidebar-footer">
          <div className="sidebar-user">
            <div className="sidebar-avatar">{(business?.name||user?.email||"U").slice(0,2).toUpperCase()}</div>
            <div className="sidebar-user-info">
              <div className="sidebar-user-name">{business?.name || 'My Business'}</div>
              <div className="sidebar-user-role">{user?.email || ''}</div>
            </div>
            <button className="sidebar-signout" title="Sign out" onClick={async () => { await signOut(); navigate('/haven/login') }}>
...
function FeedbackIcon() { return <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg> }
