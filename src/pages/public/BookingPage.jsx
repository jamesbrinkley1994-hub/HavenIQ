...
// Services loaded dynamically from the business's settings
const FALLBACK_SERVICES = ['Other / Not sure']
...
export default function BookingPage() {
  ...
  const { business, submitPublicBooking, isSlotTaken } = useStore()
  const fileRef   = useRef()

  // If accessed via slug, look up that business
  const [targetBusiness, setTargetBusiness] = useState(null)
  const [loadingBiz, setLoadingBiz]         = useState(!!slug)
  ...
  useEffect(() => {
    if (slug) {
        ...
        .then(async ({ data }) => {
          ...
          if (data?.id) {
            const { data: settings } = await supabase
              .from('business_settings').select('services').eq('business_id', data.id).single()
            if (settings?.services?.length) setBizServices(settings.services.map(s => s.name).filter(Boolean))
          }
          setLoadingBiz(false)
        })
    } else {
      ...
      if (business?.id) {
        supabase.from('business_settings').select('services').eq('business_id', business.id).single()
          .then(({ data }) => {
            if (data?.services?.length) setBizServices(data.services.map(s => s.name).filter(Boolean))
          })
      }
    }
  }, [slug, business])

  const activeBusiness = targetBusiness || business
  ...
  const handleSubmit = async () => {
    ...
    if (!activeBusiness) { setErrors({ general:'Booking page not found' }); return }

    setSubmitting(true)
    try {
      const name = `${form.firstName} ${form.lastName}`.trim()
      await submitPublicBooking(activeBusiness.id, {
        name, email: form.email, phone: form.phone || '',
        service: form.service, date: dateStr, time: selectedTime,
        notes: form.message || '', attachedFile: file || null,
      })
      setSubmitted(true)
    } catch (err) {
      ...
      <div className="booking-box">
        <div className="booking-success">
          ...
          <p style={{fontSize:13.5,color:'var(--text-muted)',textAlign:'center',maxWidth:300,lineHeight:1.65}}>
            Your booking request has been sent to {activeBusiness?.name || 'the business'}. They'll be in touch shortly to confirm.
          </p>
      ...
      <div className="booking-box">
        <div className="booking-header">
          <div className="booking-brand">
            ...
            <span style={{fontSize:13,fontWeight:700}}>{activeBusiness?.name || 'Book an appointment'}</span>
          </div>
          <div className="booking-title">Request a booking</div>
          <div className="booking-sub">Fill in your details and we'll confirm your appointment as soon as possible.</div>
        </div>
        ...
        <div className="booking-form-body">
          <div className="form-group">
            ...
            <select className={`form-input${errors.service?' error':''}`} value={form.service} onChange={set('service')}>
              ...
              {(bizServices.length > 0 ? bizServices : FALLBACK_SERVICES).map(s => <option key={s}>{s}</option>)}
            </select>
                ...
                return (
                  <button key={t} disabled={taken}
                    ...
                    style={{
                      ...
                      border: `1.5px solid ${active?'var(--primary)':taken?'var(--border)':'var(--border)'}`,
