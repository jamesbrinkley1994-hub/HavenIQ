...
// Does `time` on `dateStr` overlap with any booking in `slots`?
// Shared by the cached client-side check and the fresh live re-check
// done right before a booking is actually submitted.
function findOverlap(slots, dateStr, time) {
...
export default function BookingPage() {
  ...
  const refreshTakenSlots = async (businessId) => {
    ...
    if (error) {
      // Most likely cause: the public_booking_slots view hasn't been created
      // yet (the supabase-schema.sql migration hasn't been run). Log it so
      // it's visible in the browser console instead of silently vanishing,
      // and fall back to the original table so slot-blocking still works
      // until the migration is applied.
      console.error('[HavenIQ] public_booking_slots view unavailable — has the latest supabase-schema.sql been run? Falling back to bookings table.', error)
      const fallback = await supabase
        .from('bookings').select('date, time, service')
        .eq('business_id', businessId).in('status', ['pending', 'confirmed'])
      setTakenSlots(fallback.data || [])
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Load services, deposit toggle, working hours and manual date overrides
      const { data: settings } = await supabase
        .from('business_settings').select('services, show_deposit, working_hours, date_overrides').eq('business_id', biz.id).single()
      ...
      if (settings?.date_overrides) setDateOverrides(settings.date_overrides)
      ...
      await refreshTakenSlots(biz.id)
      setLoading(false)
    }
    load()
  }, [slug])

  const isSlotTaken = (dateStr, time) => findOverlap(takenSlots, dateStr, time)

  // Check if a date is on a closed day.
  // Manual overrides (set on the Calendar page) take priority: "blocked"
  // always closes a day, "available" always opens one, even outside normal
  // working hours. Falls back to the weekly schedule otherwise, defaulting
  // to closed if working hours haven't loaded or the day isn't configured.
  const isDateClosed = (dateStr) => {
    const override = dateOverrides?.[dateStr]
    if (override === 'blocked')   return true
        ...
        <div className="booking-form-body">
          ...
          {formType === 'book' && (
            <BookingForm
              ...
              refreshTakenSlots={refreshTakenSlots}
            />
          )}
...
// ── Booking form ──────────────────────────────────────────
function BookingForm({ business, bizServices, depositNote, isSlotTaken, isDateClosed, workingHours, dateOverrides, refreshTakenSlots }) {
  ...
  const override  = dateOverrides?.[form.date]

  // Only show times that fall within the selected day's working hours,
  // and where the service would finish before closing. Manual overrides
  // take priority: "blocked" hides every slot, "available" shows the full
  // day (respecting normal start/end times if that weekday has any set).
  const visibleTimes = !form.date ? ALL_TIMES
    : override === 'blocked' ? []
    : override === 'available'
      ? ALL_TIMES.filter(t => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true)
    try {
      // Live re-check right before saving. The cached slot list on this page
      // could be out of date (e.g. someone else just booked, or this same
      // visitor booked once already and clicked "Send another") — this
      // re-fetches the latest bookings and confirms the slot is still free
      // before we actually insert.
      const freshSlots = await refreshTakenSlots(business.id)
      if (findOverlap(freshSlots, dateStr, selectedTime)) {
        setErrors({ time: 'This slot was just taken — please choose another time' })
        setSelectedTime(null)
        setSubmitting(false)
        return
      }
  ...
  if (submitted) return (
    <SuccessScreen
      ...
      message={`Your booking request has been sent to ${business?.name}. They'll be in touch shortly to confirm. Don't forget to check your junk folder for any responses!`}
      onReset={() => { setSubmitted(false); setForm({service:'',date:'',firstName:'',lastName:'',email:'',phone:'',message:'',contactMethod:'email'}); setSelectedTime(null); setFile(null); setErrors({}); if (business?.id) refreshTakenSlots(business.id) }}
    />
  )
...
function ContactForm({ business, type }) {
  ...
  if (submitted) return (
    <SuccessScreen
      title="Message sent!"
      message={`Your ${labels[type]?.toLowerCase()} has been sent to ${business?.name}. They'll be in touch soon. Don't forget to check your junk folder for any responses!`}
