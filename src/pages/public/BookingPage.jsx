...
export default function BookingPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Load services + deposit policy
      const { data: settings } = await supabase
        .from('business_settings').select('services').eq('business_id', biz.id).single()
...
// ── Booking form ──────────────────────────────────────────
function BookingForm({ business, bizServices, depositNote, isSlotTaken }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/notify`, {
        ...
        body: JSON.stringify({ type:'new_booking', businessId:business.id, booking:{customer_name:name,customer_email:form.email,customer_phone:form.phone,service:form.service,date:dateStr,time:selectedTime,notes:form.message} }),
      }).catch(()=>{})
      setSubmitted(true)
  ...
  if (submitted) return (
    <SuccessScreen
      ...
      onReset={() => { setSubmitted(false); setForm({service:'',date:'',firstName:'',lastName:'',email:'',phone:'',message:''}); setSelectedTime(null); setFile(null); setErrors({}) }}
    />
  )
