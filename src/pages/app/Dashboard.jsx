...
export default function Dashboard() {
  ...
  // Load which bookings were previously dismissed, so "Clear" survives a refresh
  useEffect(() => {
    if (!business?.id) return
    supabase.from('business_settings').select('dismissed_booking_ids').eq('business_id', business.id).maybeSingle()
      .then(({ data }) => { if (data?.dismissed_booking_ids?.length) setClearedIds(data.dismissed_booking_ids) })
  }, [business?.id])
  ...
  const nextIndex = sortedToday.findIndex(b => toMins(b.time) >= nowMins)
        ...
        <Card>
          ...
          {sortedToday.length === 0 ? (
            ...
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {sortedToday.map((b,i) => {
                const isPast = toMins(b.time) < nowMins
                ...
                <div key={b.id} style={{
                  ...
                  background: isNext ? 'var(--primary-tint)' : 'var(--bg)',
                  border:`1px solid ${isNext?'#C4BFFF':'var(--border)'}`,
                  borderRadius:9,
                }}>
                  <div style={{
                    fontSize:11,fontWeight:700,color:isNext?'var(--primary)':'var(--text-muted)',
                  ...
                  {isNext&&<span style={{fontSize:10,fontWeight:700,color:'var(--primary)',background:'var(--primary-tint)',padding:'2px 7px',borderRadius:10,flexShrink:0}}>Next</span>}
                </div>
                )
              })}
            </div>
