...
export default function Calendar() {
  ...
  const key = d => `${viewYear}-${viewMonth}-${d}`
  ...
  const handleDayClick = (d,dow) => {
    ...
    if (mode && !isWeekend) {
      ...
      setManualDays(prev => {
        ...
        if(mode==='block')     return {...prev,[key(d)]:cur==='blocked'?null:'blocked'}
        if(mode==='available') return {...prev,[key(d)]:cur==='available'?null:'available'}
        return prev
      })
    } else {
